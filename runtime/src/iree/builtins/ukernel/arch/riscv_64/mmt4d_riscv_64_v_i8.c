...
iree_uk_mmt4d_tile_s8s8s32_4x4x8_riscv_64_xsmtvdot_native(
  ...
  for (int k = 0; k < params->K; ++k) {
    const iree_uk_int8_t* lhs_k_ptr = lhs_ptr + k * 32;
    const iree_uk_int8_t* rhs_k_ptr = rhs_ptr + k * 32;
    asm volatile(
        "vle8.v v0, (%0)\n\t"
        "vle8.v v1, (%1)\n\t"
        ".insn r 0x2b, 0x3, 0x71, x28, x0, x1\n\t"
        :
        : "r"(lhs_k_ptr), "r"(rhs_k_ptr)
        : "memory");
  }
...
IREE_UK_ATTRIBUTE_ALWAYS_INLINE static inline void
iree_uk_riscv_64_xopu_opmvinbcast_mc0_from_v0(void) {
  asm volatile(".insn r 0x57, 0x6, 0x59, x16, x0, x0\n\t" : : : "memory");
}
...
iree_uk_riscv_64_xopu_vopacc_m0_v16_v18(void) {
  asm volatile(".insn r 0x57, 0x2, 0x51, x0, x18, x16\n\t" : : : "memory");
}
...
IREE_UK_ATTRIBUTE_ALWAYS_INLINE static inline void
iree_uk_mmt4d_tile_s8s8s32_16x16x128_riscv_64_xopu_native(
    iree_uk_int32_t* IREE_UK_RESTRICT out_ptr,
    const iree_uk_int8_t* IREE_UK_RESTRICT lhs_ptr,
    const iree_uk_int8_t* IREE_UK_RESTRICT rhs_ptr,
    const iree_uk_mmt4d_params_t* params) {
  const long avl16 = 16;
  const long stride = 128;
  ...
  // OPU keeps the accumulator in m0; initialize from output tile if needed.
  asm volatile("vmv.v.i v0, 0\n\t" : : : "memory");
  iree_uk_riscv_64_xopu_opmvinbcast_mc0_from_v0();
  if (params->flags & IREE_UK_FLAG_MMT4D_ACCUMULATE) {
  ...
  for (int k = 0; k < params->K; ++k) {
    const iree_uk_int8_t* lhs_k_ptr = lhs_ptr + k * 16 * 128;
    const iree_uk_int8_t* rhs_k_ptr = rhs_ptr + k * 16 * 128;
    ...
    for (int k0 = 0; k0 < 128; ++k0) {
      asm volatile("vlse8.v v16, (%0), %1\n\t"
                   :
                   : "r"(lhs_k_ptr + k0), "r"(stride)
                   : "memory");
      asm volatile("vlse8.v v18, (%0), %1\n\t"
                   :
                   : "r"(rhs_k_ptr + k0), "r"(stride)
                   : "memory");
...
IREE_UK_ATTRIBUTE_ALWAYS_INLINE static inline void
iree_uk_mmt4d_tile_s8s8s32_1xXXx128_to_16xXXx128_riscv_64_xopu_fallback(
    void* IREE_UK_RESTRICT out_tile, const void* IREE_UK_RESTRICT lhs_panel,
    const void* IREE_UK_RESTRICT rhs_panel,
    const iree_uk_mmt4d_params_t* params, int M0) {
  IREE_UK_ASSERT(M0 >= 1 && M0 <= 16);
  ...
  for (int m0 = 0; m0 < M0; m0 += 4) {
    ...
    for (int k = 0; k < params->K; ++k) {
      ...
      const iree_uk_int8_t* lhs_k_ptr = lhs_ptr + k * M0 * K0 + m0 * K0;
      for (int k0 = 0; k0 < K0; ++k0) {
        vint8m1_t rhs = __riscv_vlse8_v_i8m1(rhs_k_ptr + k0, K0, vl);
        const iree_uk_int8_t* lhs_col = lhs_k_ptr + k0;
        iree_uk_int8_t lhs0 = lhs_col[0];
        vint16m2_t prod0 = __riscv_vwmul_vx_i16m2(rhs, lhs0, vl);
        acc0 = __riscv_vwadd_wv_i32m4(acc0, prod0, vl);
        if (rows >= 2) {
          iree_uk_int8_t lhs1 = lhs_col[K0];
          vint16m2_t prod1 = __riscv_vwmul_vx_i16m2(rhs, lhs1, vl);
          acc1 = __riscv_vwadd_wv_i32m4(acc1, prod1, vl);
        }
        if (rows >= 3) {
          iree_uk_int8_t lhs2 = lhs_col[2 * K0];
          vint16m2_t prod2 = __riscv_vwmul_vx_i16m2(rhs, lhs2, vl);
          acc2 = __riscv_vwadd_wv_i32m4(acc2, prod2, vl);
        }
        if (rows >= 4) {
          iree_uk_int8_t lhs3 = lhs_col[3 * K0];
          vint16m2_t prod3 = __riscv_vwmul_vx_i16m2(rhs, lhs3, vl);
          acc3 = __riscv_vwadd_wv_i32m4(acc3, prod3, vl);
        }
      }
